...
export function calculatePersonSummaries(
  ...
  const summaries = people.map((person) => {
    ...
    return {
      ...
      taxAmount,
      serviceAmount,
      total: round2(subtotal + taxAmount + serviceAmount),
      items,
    };
  });

  // Adjust rounding remainder: ensure sum of per-person tax/service equals the total
  ...
  if (summaries.length > 0) {
    ...
    if (taxDrift !== 0) {
      ...
      summaries[0].total = round2(summaries[0].subtotal + summaries[0].taxAmount + summaries[0].serviceAmount);
    }
    if (serviceDrift !== 0) {
